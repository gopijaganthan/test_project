...
class Base_Page(Borg,unittest.TestCase):
    ...
    def __init__(self,base_url='https://madstreetden.myshopify.com/',trailing_slash_flag=True):
        "Constructor"
        Borg.__init__(self)
    ...
    def get_attribute(self,locator,attribute):
        "Return the text for specific attribute"
        text = ''
        try:
            text = self.get_element(locator).get_attribute(attribute)
        except Exception as e:
            self.write(e)
            self.exceptions.append("Error when getting text from the attribute-'%s' in the conf/locators.conf file"%locator)
            return None
        else:
            return text
    ...
    def get_texts(self,locator):
        ...
        text = ''
        try:
            texts = [el.text for el in self.get_elements(locator)]
        except Exception as e:
            self.write(e)
            self.exceptions.append("Error when getting text from the path-'%s' in the conf/locators.conf file"%locator)
            return None
    ...
    def get_driver_requests_made(self):
        "Execute javascipt"
        requests_made = ''
        try:
            requests_made = self.get_current_driver().requests
            result_flag = True
        except Exception as e:
            result_flag = False
